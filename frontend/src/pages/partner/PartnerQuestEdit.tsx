...
export default function PartnerQuestEdit() {
  ...
  useEffect(() => {
    if (!isNew) {
      api.get(`/partner/quests/${id}`).then(r => {
  ...
  async function saveSteps() {
    ...
    setSavingSteps(true)
    try {
      await api.put(`/partner/quests/${id}/steps`, steps.map((s, i) => ({ ...s, order: i + 1 })))
    } catch {
  ...
  async function save(asDraft: boolean) {
    ...
    try {
      ...
      let savedId = id
      if (isNew) {
        const r = await api.post('/partner/quests', data)
        savedId = r.data._id || r.data.id
      } else {
        await api.patch(`/partner/quests/${id}`, data)
      }
      if (coverFile && savedId && savedId !== 'new') {
        const fd = new FormData()
        fd.append('image', coverFile)
        await api.post(`/partner/quests/${savedId}/cover`, fd)
      }
      if (!isNew && savedId && steps.length > 0) {
        await api.put(`/partner/quests/${savedId}/steps`, steps.map((s, i) => ({ ...s, order: i + 1 })))
      }
      navigate('/partner/quests')
    } catch {
  ...
  const statusInfo = STATUS_INFO[form.status] || STATUS_INFO.draft
    ...
    <div className="min-h-screen bg-gray-50 pb-48">
      <div className="bg-white px-4 pt-5 pb-4 flex items-center gap-3 sticky top-0 z-10 border-b border-gray-100">
        ...
        <div className="flex-1">
          <p className="text-xs text-gray-400 font-medium">Редактор квеста</p>
          <h1 className="text-lg font-extrabold leading-tight">{isNew ? 'Новый квест' : form.title || 'Редактировать'}</h1>
        </div>
        {!isNew && (
          <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${statusInfo.cls}`}>
            {statusInfo.label}
          </span>
        )}
      </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3 px-1">
              <div>
                ...
                <h2 className="text-base font-extrabold leading-tight">{steps.length > 0 ? `${steps.length} шагов` : 'Нет шагов'}</h2>
              </div>
                    ...
                    <div className="border-t border-gray-50">
                      ...
                      <div className="px-4 py-3 border-b border-gray-50">
                        ...
                        <select value={step.type} onChange={e => {
                          ...
                          <option value="location_info">Описание локации</option>
                        ...
                        {step.image ? (
                          ...
                          <label className="flex items-center gap-2 border border-dashed border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-[#FFD600] transition-colors">
                            ...
                            <input type="file" accept="image/*" className="hidden" onChange={async e => {
                              ...
                              try {
                                const r = await api.post(`/partner/quests/${id}/step-image`, fd)
        ...
        {!isNew && form.status !== 'published' && (
          <div className="flex items-center gap-3 bg-blue-50 rounded-2xl px-4 py-3">
            ...
            <p className="text-xs text-blue-700 font-medium leading-snug">
              После отправки на проверку квест будет опубликован администратором
            </p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-20 left-0 right-0 bg-white/90 backdrop-blur border-t border-gray-100 px-4 py-3.5 flex gap-3 z-40">
        ...
        <button onClick={() => save(false)} disabled={saving || form.status === 'published'}
          className="flex-2 bg-[#FFD600] text-black font-bold rounded-full px-6 py-3 text-sm disabled:opacity-50 active:scale-[0.98] transition-transform">
          {saving ? 'Сохраняем...' : form.status === 'published' ? 'Опубликован' : 'На проверку'}
        </button>
      </div>
